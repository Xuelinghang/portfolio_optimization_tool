...
def get_portfolio_metrics(portfolio_id):
    ...
    # Build a mapping of ticker to company name from portfolio data
    names = { item.get("ticker"): item.get("name", item.get("ticker")) for item in portfolio_data if item.get("ticker") }
    ...
    metrics_results = calculate_portfolio_metrics(p_data, returns_data)
    ...
    formatted_metrics = {
        ...
        "annual_returns": {str(year): float(value * 100) for year, value in metrics_results['annual_returns'].items()},
        "trailing_returns": {period: float(value * 100) for period, value in metrics_results['trailing_returns'].items()},
        "asset_metrics": {
            ticker: {
                "CAGR": float(metrics['cagr'] * 100),
                "Standard Deviation": float(metrics['std_dev'] * 100),
                "Maximum Drawdown": float(metrics['max_drawdown'] * 100),
